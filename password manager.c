...
BY MUHAMMAD SAAD SULEMAN AND SYED AYAAN ALI
ROLL NO. FA21-BCS-053 AND ROLL NO. FA21-BCS-119
*/
...
char inVault()
{
    ...
    printf("\n1 to view saved credentials by website name\n2 for saving new credentials\n3 for view all the credentials");
    printf("\nTo go back: press -1");
    printf("\n\nEnter your choice:  ");
    scanf("%d", &newChoice);
    if (newChoice == -1)
...
void signUpScreen()
{
    ...
    char questionn[SIZE];
    int VaultLimitCheck=0;
    char VaultLimitStorage[SIZE];
    FILE *fileptr;
    char characterStore;
    char characterStore2='g';
    fileptr=fopen("Accountlimit.txt","r");
    if(fileptr==NULL)
        {
            printf("Error File Not found");
        }
    if(fileptr!=NULL){
    characterStore2=getc(fileptr);
    }
    fclose(fileptr);

    if(characterStore2 != '1' && characterStore2 != '2' && characterStore2 != '3' && characterStore2 != '4' && characterStore2 != '5' && characterStore2!='g' && characterStore2 !='0' )
        {
            printf("\nThe maximum number of Vaults that can be created are 5\nYou have reached your limit.");
            printf("\nDelete a previous vault first to create another one\n");
    ...
    do{
        if(strlen(UserName)<6)
            {
                printf("\n\nMinimum length of the Username is 6 characters long.");
            }
    printf("\n\nEnter the Username(MIN length 6): ");
    scanf("%s", &UserName);
        ...
        if (password_choice == 1)
        {
            do{
                    if(strlen(Master_Password)<6)
                        {
                            printf("\nMIN length of Master Password should be should be 6");
                        }
            // for Master Password
            i = 0;
            printf("\nPlease Enter your Master Password (MIN length 6): ");
            while ((ch = getch()) != 13)
            {
                if (ch == 8)
                {
            ...
            Master_Password[i] = '\0';
            }while(strlen(Master_Password)<6);
            // to show the password
            printf("\n\nDo you want to show the Master password now: 1 for yes/2 for no? ");
            printf("Your reply: ");
            scanf("%d", &n);
...
fileptr=fopen("Accountlimit.txt","r");
fgets(VaultLimitStorage,sizeof(VaultLimitStorage),fileptr);
fclose(fileptr);
fileptr=fopen("Accountlimit.txt","w");
if(strcmp(VaultLimitStorage,"1")==0)
    {


fprintf(fileptr,"2");
    } else if(strcmp(VaultLimitStorage,"2")==0)
    {

fprintf(fileptr,"3");
    } else if(strcmp(VaultLimitStorage,"3")==0)
    {

fprintf(fileptr,"4");
    } else if(strcmp(VaultLimitStorage,"4")==0)
    {

fprintf(fileptr,"5");
    }
    else if(strcmp(VaultLimitStorage,"5")==0)
    {
...
void DeleteUser()
{
    char stringstore[SIZE];
    FILE *fptr;
    system("cls");
    printf("Enter the Username of the Vault you want to delete: ");
    scanf("%s",UserName);
    char *extension=".txt";
    char filename[strlen(extension)+strlen(UserName)+1];
    snprintf(filename,sizeof(filename),"%s%s",UserName,extension);
    remove(filename);
    fptr=fopen("Accountlimit.txt","r");
    fgets(stringstore,sizeof(stringstore),fptr);
    fclose(fptr);
    fptr=fopen("Accountlimit.txt","w");
    if(strcmp(stringstore,"6")==0)
        {
            fprintf(fptr,"5");
        } else if(strcmp(stringstore,"5")==0)
        {
            fprintf(fptr,"4");
        } else if(strcmp(stringstore,"4")==0)
        {
            fprintf(fptr,"3");
        } else if(strcmp(stringstore,"3")==0)
        {
            fprintf(fptr,"2");
        } else if(strcmp(stringstore,"2")==0)
        {
            fprintf(fptr,"1");
        } else if(strcmp(stringstore,"1")==0)
        {
            fprintf(fptr,"0");
        }
        else if(strcmp(stringstore, "0") == 0){
            fprintf(fptr,"0");
        }
    fclose(fptr);
    rewind(fptr);
    ...
    while(fgets(currentLine, sizeof(currentLine), filePointer))
                        ...
                        if ((p = strchr(currentLine, '\n')) != NULL)
        {
            *p = '\0';
                        ...
                        if((e=strchr(currentLine,'_'))!=NULL)
                            {
                                *e='\0';
                            }
